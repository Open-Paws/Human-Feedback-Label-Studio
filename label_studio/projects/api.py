...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        tags=['Projects'],
        operation_summary='Filter projects by language',
        operation_description="""
        Retrieve a list of projects filtered by language. You can filter projects by specifying a language in the title.
        """,
        manual_parameters=[
            openapi.Parameter(
                'language',
...
class ProjectLanguageFilterAPI(generics.ListAPIView):
    parser_classes = (JSONParser, FormParser, MultiPartParser)
    serializer_class = ProjectSerializer
    filter_backends = [filters.OrderingFilter, DjangoFilterBackend]
    filterset_class = ProjectFilterSet
    ...
    def get_queryset(self):
        ...
        if language:
            queryset = queryset.filter(title__icontains=language)
        return queryset
